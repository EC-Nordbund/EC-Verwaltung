...
ec-wrapper(
  ...
  v-list(lines='two')
    v-list-item(
      v-for='item in data.filter(filterGenerator(suche))',
      :key='item.akID',
      @click='navigate(`/ak/${item.akID}`)'
    )
      template(#prepend)
        v-icon group
      v-list-item-title {{ item.bezeichnung }}
      v-list-item-subtitle ID: {{ item.akID }}
  template(#dialogs)
...
// Ersatz für das frühere Template-Global $util.filter
...
const addAK = useTemplateRef<InstanceType<typeof FormularSelector>>('addAK')
